...
let ApiServiceService = class ApiServiceService {
    ...
    loadLiveProgram() {
        ...
        return this.programClient.call("LoadLiveProgram", null).pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_1__.map)((data) => {
            return JSON.parse(data.result.LoadLiveProgramResult);
        }));
    }
    loadUpcomingProgram() {
        this.programClient.addHttpHeader("Content-Type", "text/xml");
        return this.programClient.call("LoadUpcomingProgram", { Count: 10 }).pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_1__.map)((data) => {
    ...
    loadMonthlyProgram() {
        this.programClient.addHttpHeader("Content-Type", "text/xml");
        return this.programClient.call("PopularProgramOfMonth", {}).pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_1__.map)((data) => {
...
let AudienceRegistration2Component = class AudienceRegistration2Component {
  constructor(fb, apiService, router, localStorage, utilService, config, calendar) {
    this.fb = fb;
    this.apiService = apiService;
    this.router = router;
    this.localStorage = localStorage;
    this.utilService = utilService; // customize default values of datepickers used by this component tree
    ...
    config.markDisabled = date => calendar.getWeekday(date) >= 6; // setting datepicker popup to close only on click outside


    config.autoClose = "outside"; // setting datepicker popup to open above the input
  ...
  submit(isValid, formValue) {
    ...
    this.apiService.doAudienceProfileUpdate(formObj).subscribe(res => {
      ...
      res.result == "failure" && this.utilService.presentToast(res.message);
      res.result == "success" && this.localStorage.set("audienceData", formObj) && this.utilService.presentToast("Successfull registered") && this.router.navigate([""]);
    });
    console.log("form obj", isValid, formObj);
  }
...
AudienceRegistration2Component.ctorParameters = () => [{
  ...
  type: _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_8__.NgbInputDatepickerConfig
}, {
...
AudienceRegistration2Component = (0,tslib__WEBPACK_IMPORTED_MODULE_9__.__decorate)([(0,_angular_core__WEBPACK_IMPORTED_MODULE_10__.Component)({
  ...
  providers: [_ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_8__.NgbInputDatepickerConfig],
